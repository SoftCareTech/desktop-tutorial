...
public static  String  loginUser="Not Loggged In";
...
//retrieve loggedIn user name from Login.java class field
        if(Login.loginUser!=null&&Login.loginUser.length()>0){
            //  navigation from Login.java to the Feedback.java
            Intent addComment = new Intent(Login.this,
                    Feedback.class);
            // starting the Feedback.class program
            startActivity(addComment);
        }else{
            Toast.makeText(this,"User must Logged first",Toast.LENGTH_LONG);
        }
